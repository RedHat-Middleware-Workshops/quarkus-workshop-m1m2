...
Keycloak needs a database to store its data. We'll use Postgres. Back on the {{ CONSOLE_URL }}/topology/ns/{{ USER_ID }}-project?view=graph[OpenShift web console^], select **Software Catalog**, under **Home** on the left.
