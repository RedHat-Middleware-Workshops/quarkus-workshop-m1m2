...
----
mvn quarkus:add-extension -Dextensions="smallrye-opentracing, rest-client-reactive"
----
...
Reload the https://jaeger-{{USER_ID}}-project.{{ROUTE_SUBDOMAIN}}[Jaeger Query UI^], and click **Find Traces**. The new trace should appear the top with multiple spans. Select it to display details:
