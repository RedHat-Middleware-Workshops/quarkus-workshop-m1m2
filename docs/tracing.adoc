...
Run the following `oc` command in the VS Code terminal to install a Jaeger instance in your project:

[source,sh,role="copypaste"]
----
oc apply -f src/main/kubernetes/jaeger.yaml -n {{ USER_ID }}-project
----

// From the {{ CONSOLE_URL }}/topology/ns/{{ USER_ID }}-project[Topology View^], click on **Installed Operators** in the sidebar menu and select **Jaeger**
...
Jaeger exposes its collector at different ports for different protocols. Most use the HTTP collector at `jaeger-collector:14268` but other protocols like gRPC are also supported on different ports. You can see them by clicking on the {{ CONSOLE_URL }}/k8s/ns/{{ USER_ID }}-project/services/jaeger-collector[Service details^]:
