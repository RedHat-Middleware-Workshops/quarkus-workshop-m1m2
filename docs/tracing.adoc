...
This will add the necessary entries in your `pom.xml` to bring in the OpenTracing capability. It will import the `smallrye-opentracing` extension which is an implementation of the MicroProfile OpenTracing specification used in Quarkus, along with the default https://www.jaegertracing.io/[Jaeger] tracer implementation.
...
You'll need to trigger the HTTP endpoints, so let's re-visit our earlier DataTables-powered app. Run this command to open the table with our people database:
...
This exercise showa how to use the https://github.com/eclipse/microprofile-rest-client[MicroProfile REST Client] with Quarkus in order to trace external REST APIs requests with very little effort.
...
This contains a subset of the full Star Wars REST model, just enough to demonstrate tracing.

### Create interface 

Using the MicroProfile REST Client is as simple as creating an interface using the proper JAX-RS and MicroProfile annotations. Create a new Java class in the `org.acme.people.service` package called `StarWarsService` with the following content:
...
<2> Generate a stream of integers that we will use as IDs to pass to the service
...
[source,sh,role="copypaste"]
----
echo http://$(oc get route people -o=go-template --template='{{ .spec.host }}')/person/swpeople
----
...
Go back to the Jaeger console tab, and click **Find Traces**. The new trace should appear the top with multiple spans:
