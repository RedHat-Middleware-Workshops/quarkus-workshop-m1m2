...
----
oc create -f https://raw.githubusercontent.com/RedHat-Middleware-Workshops/quarkus-workshop-m1m2-labs/refs/heads/ocp-4.19/src/main/kubernetes/kafka.yaml
----

This will cause the Operator to provision a new Kafka cluster.

Back on the {{ CONSOLE_URL }}/topology/ns/{{ USER_ID }}-project?view=graph[Topology View^], make sure all the Kafka pods are up and running (with dark blue circles):
