...
First, on the {{ CONSOLE_URL }}/topology/ns/{{ USER_ID }}-project[Topology View^], select **+Add to Project** on the left. Then, Type in `Kafka` in the search box, and select on the *Kafka*:

image::fromcat-kafka.png[names,800]

These are all of the Kafka cluster elements you can install. Select on **Create**. This will open a yaml file for you to configure the cluster before it's installed. Change the name of the cluster from `my-cluster` to `names-cluster` (under the _metadata_ section of the YAML file). Leave all other values as-is, and select **Create**:
...
Select **+Add to Project** on the left. Then, Type in `Topic` in the search box, and select on the *Create*:
