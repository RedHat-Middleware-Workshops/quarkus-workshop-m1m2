...
----
INFO  [io.quarkus] (main) people 1.0-SNAPSHOT native (powered by Quarkus 3.2.6.Final-redhat-00002) started in 0.026s. Listening on: http://0.0.0.0:8081
----

That's *19* milliseconds to start up.
...
----
    PID   RSS COMMAND
  18590  2740 sh -c cd /projects/quarkus-workshop-m1m2-labs; ./target/people-1.0-SNAPSHOT-runner -Dquarkus.http.port=8081
  18597 49332 ./target/people-1.0-SNAPSHOT-runner -Dquarkus.http.port=8081
----

The 2nd row is the actual process that is taking around 49 MB of memory (https://en.wikipedia.org/wiki/Resident_set_size[Resident Set Size^], or RSS). Pretty compact!
