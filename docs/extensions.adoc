...
Once you log in, you’ll be placed on your personal dashboard. Select on the name of the pre-created workspace on the left, as shown below (the name will be different depending on your assigned number).

image::crw-landing.png[cdw, 800]

You can also select on the name of the workspace in the center, and then select on the green {{ USER_ID}}-namespace that says _Open_ on the top right hand side of the screen:
...
Let's import our project. Select on **Clone Repository** (or type kbd:[F1], enter 'git' and select on the auto-completed _Git Clone.._ )
...
Ensure `projects/` is selected, then select on *Select Repository Location* then select on *Add to Workspace*:
...
Navigate to `src -> main -> java -> org.acme.people.rest` in the project tree and double select on `GreetingResource.java`.
...
Start the app by selecting on _Live Coding_:
...
You should see the default Quarkus welcome page (you may need to select the _reload_ icon):
...
You can also select on the URL link at the upper right to open the same default page in a separate browser tab:
...
Although your CodeReady workspace is running on the Kubernetes cluster, it's running with a default restricted _Service Account_ that prevents you from creating most resource types. So, in CodeReady, log in with your workshop user. Select on *Login to OpenShift*:

[NOTE]
====
When you select on this, CodeReady Workspaces executes the following command:
...
Once logged in, select on the name of your project (`{{ USER_ID }}-project`):
...
Select on the _Source Control_ view and then double-select on `pom.xml`:
