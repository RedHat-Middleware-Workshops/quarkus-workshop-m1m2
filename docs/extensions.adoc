...
[source,sh,role="copypaste"]
----
cd $CHE_PROJECTS_ROOT/quarkus-workshop-m1m2-labs && git checkout ocp-4.7
----
...
[source,none]
----
INFO  [io.quarkus] (Quarkus Main Thread) people 1.0-SNAPSHOT on JVM (powered by Quarkus xx.xx.xx) started in 1.102s. Listening on: http://0.0.0.0:8080
...
[source,sh,role="copypaste"]
----
mvn -q quarkus:list-extensions -f $CHE_PROJECTS_ROOT/quarkus-workshop-m1m2-labs
----

You can see the list of ~286 different extensions available to you in the output:
...
Later on in this lab we'll be using `JSON-B` serialization support for RESTEasy, so let's add that extension here. In the Terminal, run the following command to add the _RESTEasy JSON-B_ extension to your project:

[source,sh,role="copypaste"]
----
mvn -q quarkus:add-extension -Dextensions="resteasy-jsonb" -f $CHE_PROJECTS_ROOT/quarkus-workshop-m1m2-labs
----
