...
This lets you fine tune the display, along with the type of graph (bar, line, gauge, etc). Leave them for now, and click on the _Panel_ tab. Change the name of the panel to `Converted Names`.
...
* The different quantiles of time it takes to process names `application_org_acme_people_stream_NameConverter_converter_seconds` (configure it to _stack_ its values on the _Visualization_ , and name it "Converter Performance" on the _Panel Title_).
