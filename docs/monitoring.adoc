...
From the {{ CONSOLE_URL }}/topology/ns/{{ USER_ID }}-project[Topology View for your project^], click on the `+` icon in the header menu, and choose "Container Image"

image::add-container.png[Prometheus, 800]
...
Follow the same process as before: On the {{ CONSOLE_URL }}/topology/ns/{{ USER_ID }}-project[Topology View^], select the `+` icon in the header menu, and choose "Container Image".

image::add-container.png[Grafana, 800]
