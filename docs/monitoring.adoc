...
[source,sh,role="copypaste"]
----
mvn -q quarkus:add-extension -Dextensions="micrometer-registry-prometheus" -f $CHE_PROJECTS_ROOT/quarkus-workshop-m1m2-labs
----
...
Replace the `process()` method with the following code for adding the `counter` and  `timer`API:
