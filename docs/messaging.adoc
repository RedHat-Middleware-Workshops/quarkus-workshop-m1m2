...
----
<1> Be sure to import the correct `EventBus` class - which is `io.vertx.axle.core.eventbus.EventBus`. 

Next, create two new endpoints which creates new people in our database given a name, and finds people by their name:
...
With our endpoint, confirm it works using the Terminal to execute:
...
**This will fail** with a timeout - we posted the message to the Vert.x event bus, but there's nothing to receive it!
