...
--
All 1 test is passing (0 skipped), 1 test was run in 534ms. Tests completed at 17:54:41 due to changes to GreetingResource.class.
...
----
--
All 2 tests are passing (0 skipped), 2 tests were run in 567ms. Tests completed at 17:55:29 due to changes to GreetingResource.class and 1 other files.
Press [r] to re-run, [o] Toggle test output, [:] for the terminal, [h] for more options>
----
...
Unfold the `ENDPOINTS > Public` and select the clipboard icon to copy the *Dev UI* URL.
...
As you see, `All tests passed` on the right bottom. When you select on `Open` on the left bottom, it will show the Quarkus runtime logs:
...
Select on `Test results` then it will show the details of the test results:
...
[source, none]
----
All 3 tests are passing (0 skipped), 1 test was run in 779ms. Tests completed at 18:16:37 due to changes to StaticContentTest.class.
----
...
[source, none]
----
All 4 tests are passing (0 skipped), 1 test was run in 662ms. Tests completed at 18:17:54 due to changes to MockGreetingService.class and 1 other files.
----
