...
Acess the https://{{ USER_ID }}-quarkus-workshop-index-webpage.{{ ROUTE_SUBDOMAIN }}/q/dev-ui/[Quarkus Dev UI^]
...
Right-click on this directory and select _New -> File_. Name the file `test.html` in the dialog box:
...
[source, none]
----
All 3 tests are passing (0 skipped), 1 test was run in 556ms. Tests completed at 01:37:50 due to changes to MockGreetingService.class and 1 other files.
----
