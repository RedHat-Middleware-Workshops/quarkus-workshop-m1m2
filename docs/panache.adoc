...
----
mvn quarkus:add-extension -Dextensions="hibernate-orm-panache, jdbc-h2, jdbc-postgresql"
----
...
Our production environment needs a "real" database so let's deploy a Postgres database to OpenShift. Back in the {{ CONSOLE_URL }}/topology/ns/{{ USER_ID }}-project[Topology View^], select **+Add to Project** on the left. Then, Type in `PostgreSQL (Ephemeral)` in the search box, and select on the *PostgreSQL (ephemeral)*:

image::db.png[db, 700]

Select on *Instantiate Template* and fill in the following fields, leaving the others as their default values:
...
This will deploy the database to our new project. Select on the {{ CONSOLE_URL }}/topology/ns/{{ USER_ID }}-project[Topology View^] to see it:
