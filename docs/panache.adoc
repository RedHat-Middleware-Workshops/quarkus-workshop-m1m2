...
You may have wondered why we are using `people` in our Java APIs. We'll be creating a simple data-driven application that manages a set of people, who have a name, birth date, and eye color. In the previous lab we added the extension for Panache to our project, but we will need a few more for connecting to databases. Add these extensions with the following command:

[source,sh,role="copypaste"]
----
mvn quarkus:add-extension -Dextensions="panache, h2, postgresql"
----

We'll use the in-memory H2 database for local testing, and Postgres when we deploy to our production Kubernetes environment. So that's why we have added in the `h2` and `postgresql` extensions.

The endpoints (JDBC url, database credentials) are specified in `application.properties` using the Quarkus Profile support. So for example you'll find this:
...
This causes the `%dev` properties to be active in development (and the `%test` in test mode), and the `%prod` properties to be used when in production (not in dev or test).
...
Open the `PersonResource` class and add a new endpoint under the `//TODO: add basic queries` comment:
...
This code will insert 10,000 fake people with random birthdates, eye colors, and names at startup. Note the use of the `@Transactional` annotation - this is required for methods that make changes to the underlying database (and automatically executes the method in a Transaction for you).
...
[NOTE]
====
Adding 10k entries will make startup time artificially high, around 5-10 seconds. 
====
...
With our database running, re-build the application as an executable JAR using the command palette and selecting **Package Executable JAR**.

Next, re-define the container build to use the OpenJDK image:
...
We'll need to re-build the app as an Executable JAR. Using the command palette, select **Build Executable JAR**. You should see a bunch of log output that ends with a `SUCCESS` message.
