...
This causes the `%dev` properties to be active in development (and test) mode, and the `%prod` properties to be used when in production (not in dev or test).
...
Now we are ready to run our application. Using the command palette, select **Build and Run Locally**. You should see a bunch of log output that ends with:
