...
[NOTE]
====
Adding 1k entries will make startup time artificially high, around 3-5 seconds. 
====
...
Next, re-define the container build to use the OpenJDK image using these two commands:

[source,sh,role="copypaste"]
----
oc delete bc/people 
oc new-build registry.access.redhat.com/redhat-openjdk-18/openjdk18-openshift:1.5 --binary --name=people
oc set env dc/people JAVA_OPTIONS='-Dfile.encoding=UTF8' # <1>
----
<1> We need to set the file encoding to handle the special characters in our sample data
